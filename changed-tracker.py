...
while img_counter < n_frames:
    ...
    if img_counter == 0:
        init_img = frame
        # init_img = cv2.imread('./lk_tracker/box/frame00001.jpg', 0)
        fig = plt.figure()
        ax = fig.add_subplot()
        ax.imshow(init_img)
        ...
        init_corners_cv = (init_corners[0, 0], init_corners[1, 0], init_corners[0, 3] - init_corners[0, 0], init_corners[1, 3] - init_corners[1, 0])
        print(init_corners)
        kcf_t.init(init_img, init_corners_cv)
        print("done..")
    else:
        ok, tracker_corners = kcf_t.update(frame)
        print(tracker_corners)
        ...
        x_line = [tracker_corners[0], tracker_corners[0] + tracker_corners[2]]
        y_line = [tracker_corners[1], tracker_corners[1]]
        ax.plot(x_line, y_line, color='blue')
        x_line = [tracker_corners[0], tracker_corners[0]]
        y_line = [tracker_corners[1], tracker_corners[1] + tracker_corners[3]]
        ax.plot(x_line, y_line, color='blue')
        x_line = [tracker_corners[0], tracker_corners[0] + tracker_corners[2]]
        y_line = [tracker_corners[1]+ tracker_corners[3], tracker_corners[1]+ tracker_corners[3]]
        ax.plot(x_line, y_line, color='blue')
        x_line = [tracker_corners[0]+ tracker_corners[2], tracker_corners[0]+ tracker_corners[2]]
        y_line = [tracker_corners[1], tracker_corners[1] + tracker_corners[3]]
        ax.plot(x_line, y_line, color='blue')
        ...
        buf = canvas.buffer_rgba()
        image = np.asarray(buf)
        cv2.imwrite("./tracker_output/" + str(img_counter) + ".jpg", image)
